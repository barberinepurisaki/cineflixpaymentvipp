...
const CheckoutReceipt = () => {
  ...
  const handlePayment = () => {
    if (hasUpsells) {
      const upsellNames = selectedUpsells.map(u => u?.name).filter(Boolean).join(', ');
      const message = `Olá! Quero assinar o ${plan.name} + ${upsellNames}. Total: R$ ${total.toFixed(2)}`;
  ...
  return (
    <div className="min-h-screen bg-cinema-dark flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="h-4 bg-white/95 rounded-t-lg" style={tornEdge} />
        ...
        <div className="bg-white/95 px-6 py-8 font-mono text-gray-900 text-sm">
          ...
          {hasUpsells && (
            <>
              <div className="border-t border-dashed border-gray-400 my-4" />
              <div className="space-y-2 mb-4">
                <p className="text-xs text-gray-500 mb-2">ADICIONAIS:</p>
                {selectedUpsells.map((u) => u && (
                  <div key={u.id} className="flex justify-between">
                    <span className="text-gray-600 text-xs">{u.name}</span>
                    <span className="text-xs">+R$ {u.price.toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </>
          ...
          <div className="text-center py-3">
            ...
            <p className="text-3xl font-bold text-gray-900">R$ {total.toFixed(2)}</p>
          </div>
          ...
          <button
            ...
            className="block w-full py-4 rounded-lg bg-red-600 hover:bg-red-500 text-white text-center font-bold text-base transition-all duration-300 hover:scale-[1.02] shadow-lg"
          >
            {hasUpsells ? '🎯 FINALIZAR SEU PEDIDO AGORA' : '💳 FINALIZAR SEU PEDIDO AGORA'}
          </button>
        ...
        <div className="h-4 bg-white/95 rounded-b-lg" style={tornBottomEdge} />
